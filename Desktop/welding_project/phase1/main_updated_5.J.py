"""
Robotic Laser Welding — Phase 3   (main_updated_5.I.py)
========================================================
5.G → 5.H eklenenler / fixler:
  • Bridging duplicate detection BUG fix:
    Konsantrik dairelerde (cylinder-in-hole gibi) Path A ve Path B'nin
    start/end noktaları çok yakın olabilir (radyal fark sadece 0.2mm).
    5.G'de bu Path B'yi Path A'nın duplicate'i sanıyordu → eklenmesini
    engelliyordu. 5.H'de duplicate check artık SIDE-AWARE: sadece aynı
    tarafın (Bridging-A vs Bridging-A) zaten eklenmiş olması durumunda
    duplicate sayılır. Path A ekli → Path B yine eklenebilir.

  • Dialog default state: Qt.Checked (5.G'deki Qt.Unchecked değişti)
    Mantık: clearance fit'te N path bulunduysa, tipik kullanıcı
    hepsini eklemek ister. Deselect etmek tıklamayı azaltır.

  • UI terminolojisi: "path" → "segment" / "weld segment"
    Dialog title "Bridging Segments", item label "Body N side weld segment".
    İç kod "Bridging-A" / "Bridging-B" method etiketlerini koruyor.

Algoritma sırası (5.H):
  1) Topological shared-edge
  2) Geometric coincident-edge
  3) Section fallback
  4) Path-based bridging proposal (side-aware duplicate filter)
  5) Hata: Manual mode öner

5.H → 5.I eklenenler:
  • Two-tier data model: Welds (kaynak işlemleri) → Segments (geometrik fragmanlar)
  • Yan panelde Welds listesi: + New / Rename / Delete butonlarıyla
  • Aktif weld'in segmentleri alt listede; weld değişince segment listesi yenilenir
  • Touching + edge yok durumu artık clearance fit dialog'u tetiklemez
    (gerçek "No Shared Edge" mesajı gösterilir)
  • Method etiketleri operatör diline çevrildi
    (Topological shared → "Shared edge", Bridging-A → "Bridging (Body A side)" vb.)
  • Gap mesafesine göre 3 ayrı dialog: touching/no-edge, gap/no-path, faces-too-far
  • JSON şeması: "segments" → "welds[].segments" (geriye dönük uyumlu)
  • Aynı face çifti aktif weld'de zaten varsa "yeni weld olarak ekle?" sorusu

Veri modeli:
  self.welds = [
    {"id": 1, "name": "Top weld", "segments": [seg1, seg2, ...]},
    ...
  ]
  self.active_weld_id = 1
  self.collected_segments → self._active_segments() proxy
"""
...
class MainWindow(QMainWindow):
    ...
    def _face_pick_score(self, face):
        area, _ = _face_info(face)
        curved_bonus = 0.0
        if BRepAdaptor_Surface is not None:
            try:
                surface = BRepAdaptor_Surface(face)
                if surface.GetType() != GeomAbs_Plane:
                    curved_bonus = area * 10.0
            except Exception:
                pass
        return curved_bonus + area

    def _face_contains_edge(self, face, edge):
    ...
    def _face_from_edge_pick(self, edge):
        candidates = []
        for body_idx, solid in enumerate(self.solids, 1):
            exp = TopExp_Explorer(solid, TopAbs_FACE)
            while exp.More():
                face = exp.Current()
                if self._face_contains_edge(face, edge):
                    candidates.append((self._face_pick_score(face), body_idx, face))
                exp.Next()
        if not candidates:
            return None, None
        _, body_idx, face = max(candidates, key=lambda item: item[0])
        return face, body_idx

    def _shape_as_face(self, shape):
        if shape.ShapeType() == TopAbs_FACE:
            return shape, None
        if shape.ShapeType() == TopAbs_EDGE:
            return self._face_from_edge_pick(shape)
        ...
        if len(faces) == 1:
            return faces[0], None
        if len(faces) > 1:
            # Some curved picks (cone/cylinder shells) come back as a shape that
            # contains several faces instead of the clicked sub-face. Prefer
            # analytic/curved surfaces, then dominant area.
            return max(faces, key=self._face_pick_score), None
        return None, None

    def _pick_face(self, face):
        face, picked_body_idx = self._shape_as_face(face)
        if face is None:
            print("[PICK] Ignored selection: no unique face found in picked shape")
            return
        body_idx = picked_body_idx or self._find_body_of_face(face)
        if body_idx is None:
            print("[PICK] Ignored face: could not map selected face to a body")
            self.lbl_status.setText(
                "Selected surface could not be mapped to a body.\n"
                "Try clicking closer to the middle of the face."
            )
            return
    ...
    def _find_body_of_face(self, face):
        target_area, target_center = _face_info(face)
        best = None
        for i, solid in enumerate(self.solids):
            ...
            while exp.More():
                ...
                if current.IsSame(face):
                    return i + 1
                area, center = _face_info(current)
                area_tol = max(0.01, target_area * 1e-4)
                center_dist = _pnt_dist(target_center, center)
                if abs(area - target_area) <= area_tol and center_dist < 1e-3:
                    score = center_dist + abs(area - target_area)
