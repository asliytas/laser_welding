...
try:
    from trajectory_mapping_1S import (
        WeldPoint,
        WeldTrajectory,
...
# ---------------------------------------------------------------------------
# Corner orientation smoothing (Fix #2 for the L-joint orientation jumps).
# When two consecutive setpoints have an orientation delta above the threshold,
# we insert N intermediate setpoints with quaternion SLERP and linear position
# interpolation. This converts a hard 90°/180° jump into a smooth ramp the
# robot controller can actually follow.
# ---------------------------------------------------------------------------

CORNER_BLEND_THRESHOLD_DEG = 30.0
CORNER_BLEND_STEPS = 5
...
def _quat_slerp(q1, q2, t):
    """Shortest-arc SLERP between two unit quaternions (w, x, y, z)."""
...
def smooth_corner_orientations(program, threshold_deg=CORNER_BLEND_THRESHOLD_DEG,
                               n_blend_steps=CORNER_BLEND_STEPS):
    """Insert SLERP-blended setpoints wherever consecutive weld setpoints exceed threshold_deg."""
    if n_blend_steps < 1 or threshold_deg <= 0.0:
        return 0
    setpoints = list(program.setpoints)
    if len(setpoints) < 2:
        return 0

    blended_total = 0
    new_setpoints = [setpoints[0]]
    for i in range(1, len(setpoints)):
        prev = new_setpoints[-1]
        curr = setpoints[i]
        if prev.phase != "weld" or curr.phase != "weld":
            new_setpoints.append(curr)
            continue
        q_angle = _quat_angle_deg(prev.pose.quaternion, curr.pose.quaternion)
        if q_angle <= threshold_deg:
            new_setpoints.append(curr)
            continue
        # Sharp corner: insert n_blend_steps intermediate poses with SLERP + linear position.
        for k in range(1, n_blend_steps + 1):
            t = k / (n_blend_steps + 1)
            pos = (
                prev.pose.position[0] + t * (curr.pose.position[0] - prev.pose.position[0]),
                prev.pose.position[1] + t * (curr.pose.position[1] - prev.pose.position[1]),
                prev.pose.position[2] + t * (curr.pose.position[2] - prev.pose.position[2]),
            )
            q = _quat_slerp(prev.pose.quaternion, curr.pose.quaternion, t)
            R = quaternion_to_rotation_matrix(q)
            euler = rotation_matrix_to_euler_zyx(R)
            arc = prev.arc_length + t * (curr.arc_length - prev.arc_length)
            blend_pose = TCPPose(
                position=pos,
                rotation_matrix=R,
                quaternion=q,
            ...
            new_setpoints.append(TrajectorySetpoint(
                index=0,  # renumbered below
                arc_length=arc,
                phase="weld",
                pose=blend_pose,
                notes=[f"corner blend {k}/{n_blend_steps} (slerp, jump {q_angle:.1f} deg)"],
            ))
        blended_total += 1
        new_setpoints.append(curr)

    for idx, sp in enumerate(new_setpoints):
        sp.index = idx
    program.setpoints[:] = new_setpoints
    if blended_total:
        program.notes.append(
            f"Corner smoothing: inserted {n_blend_steps} SLERP setpoints at "
            f"{blended_total} corner(s) (threshold {threshold_deg:.0f} deg)"
        )
    return blended_total
...
def plan_weld(trajectory, tcp=None,
              smooth_corners=True,
              corner_threshold_deg=CORNER_BLEND_THRESHOLD_DEG,
              corner_blend_steps=CORNER_BLEND_STEPS):
    ...
    # Insert SLERP-blended setpoints at sharp corners BEFORE validation, so
    # the validator only flags residual jumps (smoother runs leave no warning).
    if smooth_corners:
        smooth_corner_orientations(
            program,
            threshold_deg=corner_threshold_deg,
            n_blend_steps=corner_blend_steps,
        )
...
class TrajectoryPlannerWindow(QMainWindow if QT_AVAILABLE else object):
    ...
    def open_trajectory_json(self):
        if load_trajectory_json is None:
            QMessageBox.critical(self, "Import Failed", "trajectory_mapping_1S could not be imported.")
            return
...
def _run_cli(args):
    if load_trajectory_json is None:
        raise RuntimeError("trajectory_mapping_1S could not be imported")
