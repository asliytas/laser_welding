"""
Robotic Laser Welding — Phase 3   (main_updated_5.C.py)
========================================================
5.B → 5.C eklenenler:
...
# ── Parametreler ────────────────────────────────────────────────────────────
PROXIMITY_THRESHOLD_MM   = 5.0    # ↑'i aşarsa sadece uyarı (engelleyici değil)
EDGE_MIN_LENGTH          = 0.01
WIRE_TOLERANCE           = 0.5
COINCIDENT_LEN_TOL       = 0.01
COINCIDENT_ENDPOINT_TOL  = 0.1
COINCIDENT_DIST_TOL      = 0.1
CONTINUITY_GAP_TOL       = 0.5    # bu eşiğin altı "connected"
DUPLICATE_TOL            = 0.5    # endpoint eşleşmesi → duplicate
...
def _wire_endpoints(wire):
    """Wire'ın start, end noktasını ve closed olup olmadığını döner.
    Endpoints (x,y,z) tuple olarak. Hata durumunda (None, None, False)."""
...
class MainWindow(QMainWindow):
    """
    5.C eklenenler: segment management, JSON IO, keyboard shortcuts, status bar,
    face info, continuity & duplicate detection.
    """
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Robotic Laser Welding – Phase 3 (Segment Manager)")
        ...
        # face_x = (face, body_idx, ais, area, center)  — center: (x,y,z)
        self.face_a = None
        self.face_b = None
        ...
        # Each segment dict:
        # { wires, ais_list, method, length, type,
        #   start_point, end_point, is_closed }
        self.collected_segments = []

        # Loaded-from-JSON metadata (display only)
    ...
    def _create_side_panel(self):
        ...
        # --- Proximity (Görev 6: warning, not blocking)
        ...
        # --- Segments header (Görev 13: legend)
        ...
        # --- Segment list controls (Görev 1, 2)
        ...
        # --- Last result (Görev 7: Section warning)
    ...
    # ─── Validation (Görev 6: warning, not blocking) ──────────────────

    def _validate_face_proximity(self):
        """(gap, warning_msg) döner. Eskinin aksine engelleyici DEĞİL —
        warning_msg dolu olsa bile algoritma yine de çalıştırılır."""
    ...
    # ─── Segment color (red→yellow gradient) ──────────────────────────
    ...
    def _reassign_segment_colors(self):
        """Reorder/delete sonrası index bazlı renkleri yeniden ata."""
        ctx = self.viewer._display.GetContext()
    ...
    # ─── Duplicate detection (Görev 9) ────────────────────────────────

    def _is_duplicate_segment(self, candidate):
        """Aday segment'in endpoint'leri mevcut segmentlerden biriyle
        eşleşiyorsa o segmentin 1-based index'ini döner; yoksa None."""
    ...
    # ─── Continuity check (Görev 8) ───────────────────────────────────

    def _segment_gap(self, seg_a, seg_b):
        """seg_a end ile seg_b'nin start veya end arasındaki min mesafe."""
    ...
    def _continuity_summary(self):
        """(connected_count, gap_count, gaps_list) döner."""
    ...
    # ─── Segment list widget (Görev 1, 2, 7, 8) ───────────────────────
    ...
    def finish_path(self):
        ...
        for i, seg in enumerate(self.collected_segments):
            mark = " *" if seg["method"] == "Section" else ""
            lines.append(f"  {i + 1}. {seg['type']:<22} {seg['length']:>7.2f} mm  ({seg['method']}){mark}")
            if i < n - 1:
                if gap_list[i] is None:
                    lines.append("     ↓ connected")
                else:
                    lines.append(f"     ↓ gap: {gap_list[i]:.2f} mm")
        if any(s["method"] == "Section" for s in self.collected_segments):
            lines.append("")
            lines.append("  *  Section fallback used — verify visually")
        msg = "\n".join(lines)
        print(f"\n=== FINISH PATH ===\n{msg}")
