"""
Robotic Laser Welding — Phase 3   (main_updated_5.H.py)
========================================================
...
class BridgingPathsDialog(QDialog):
    ...
    def __init__(self, paths, single_side_warning=False, parent=None):
        ...
        self.setWindowTitle("Clearance Fit — Bridging Segments")
        self.paths = paths
        self.resize(640, 420)
        ...
        for p in paths:
            text = (
                f"Body {p['body_idx']} side  weld segment  |  "
            ...
            # 5.H: default checked — clearance fit'te tipik kullanıcı hepsini ekler
            item.setCheckState(Qt.Checked)
            self.list_widget.addItem(item)
        layout.addWidget(self.list_widget)

        warn = QLabel(
            "⚠ Bridging segment'leri otomatik tespit edilir.\n"
            "Hangi tarafın doğru kaynak yolu olduğunu görsel olarak doğrulayın. "
            "İstemediğiniz tarafın işaretini kaldırın."
        )
        warn.setStyleSheet(
...
class MainWindow(QMainWindow):
    """5.H: side-aware bridging duplicate, default-checked dialog."""
    ...
    def _apply_bridging_paths(self, paths, selected_indices):
        ...
        if added > 0:
            self.lbl_path.setText(
                f"⚠ Bridging: {added} segment(s) added — verify visually\n\n"
        ...
        print(f"[ALGO] Bridging segments applied: added={added}, skipped={skipped}")
    ...
    def add_segment(self):
        ...
        if not edges_info:
            ...
            if gap is not None and gap < CLEARANCE_THRESHOLD_MM:
                ...
                for p in all_paths:
                    ...
                    if dup is None:
                        ...
                        print(f"[ALGO] Side {p['side']} (Body {p['body_idx']}) "
                              f"already in segments as Segment {dup} — filtered out")
                ...
                if not new_paths:
                    sides_already = ", ".join(
                        f"Body {p['body_idx']} side (Segment {idx})"
                        for p, idx in already_added
                    )
                    QMessageBox.information(
                        self, "Already Added",
                        f"Bu face çiftinden tüm bridging segmentleri zaten "
                        f"eklenmiş:\n\n  {sides_already}\n\n"
                        f"Aynı segmenti ikinci kez eklemek için önce listeden silin.\n"
                        f"Veya farklı face'ler seçin."
                    )
                    return

                # Tek path tespit edildiyse (bir taraf hiç bulunamadı) uyarı
