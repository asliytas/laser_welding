...
class MainWindow(QMainWindow):
    ...
    COLOR_VIRTUAL_MIDLINE = Quantity_Color(0.22, 1.00, 0.08, Quantity_TOC_RGB)

    def __init__(self):
        super().__init__()
        self.setWindowTitle("Robotic Laser Welding - Phase 3/4 (Paths, Mapping & Planning) - 5.R")
        self.resize(1380, 820)

        self.viewer = WeldingViewer(self)
        self.viewer.on_shape_picked = self._on_shape_picked
        self._midline_picking = False
        self._midline_edge_picks = []
        self._midline_temp_ais = []
        self._projecting = False
        self._project_source_edge = None
        self._project_temp_ais = []
    ...
    def _create_side_panel(self):
        ...
        self.btn_virtual_midline = QPushButton("Create Virtual Midline")
        self.btn_virtual_midline.setCheckable(True)
        self.btn_virtual_midline.setToolTip("Pick two reference edges and create one synthetic midline path")
        self.btn_virtual_midline.toggled.connect(self._on_virtual_midline_toggled)
        self.btn_virtual_midline.setVisible(False)
        v.addWidget(self.btn_virtual_midline)

        self.lbl_virtual_midline_note = QLabel(
            "Creates a synthetic path for real gaps when automatic detection cannot bridge two edges."
        )
        self.lbl_virtual_midline_note.setWordWrap(True)
        self.lbl_virtual_midline_note.setStyleSheet(
            "padding: 4px; color: #2d7a20; font-size: 11px; font-style: italic;"
        )
        self.lbl_virtual_midline_note.setVisible(False)
        v.addWidget(self.lbl_virtual_midline_note)

        self.btn_project_edge_face = QPushButton("Project Edge -> Face")
        self.btn_project_edge_face.setCheckable(True)
        self.btn_project_edge_face.setToolTip("Pick one source edge, then a planar target face")
        self.btn_project_edge_face.toggled.connect(self._on_project_edge_face_toggled)
        self.btn_project_edge_face.setVisible(False)
        v.addWidget(self.btn_project_edge_face)
    ...
    def _on_mode_toggled(self, auto_checked):
        if auto_checked:
            self._cancel_virtual_midline_picking(reset_button=True)
            self._cancel_project_edge_face(reset_button=True)
            ...
            self.btn_virtual_midline.setVisible(True)
            self.btn_project_edge_face.setVisible(True)
            self.lbl_virtual_midline_note.setVisible(True)
    ...
    def _on_shape_picked(self, shape):
        ...
        elif self._projecting and self._project_source_edge is not None:
            self._handle_project_target_face_pick(shape)
        else:
    ...
    def _on_virtual_midline_toggled(self, checked):
        if checked:
            if not self.radio_manual.isChecked():
                self.btn_virtual_midline.setChecked(False)
                return
            self._cancel_project_edge_face(reset_button=True)
            self._activate_selection_mode(SEL_MODE_EDGE)
            self._midline_picking = True
            self._midline_edge_picks = []
            self._clear_virtual_midline_temp_highlights()
            self.lbl_status.setText("Virtual Midline:\npick edge 1 of 2")
            self.statusBar().showMessage("Virtual Midline: pick edge 1 of 2")
            return
        self._cancel_virtual_midline_picking(reset_button=False)

    def _on_project_edge_face_toggled(self, checked):
        if checked:
            if not self.radio_manual.isChecked():
                self.btn_project_edge_face.setChecked(False)
                return
            self._cancel_virtual_midline_picking(reset_button=True)
            self._projecting = True
            self._project_source_edge = None
            self._clear_project_temp_highlights()
            self._activate_selection_mode(SEL_MODE_EDGE)
            self.lbl_status.setText("Project to Face:\npick the source edge")
            self.statusBar().showMessage("Project to Face: pick the source edge")
            return
        self._cancel_project_edge_face(reset_button=False)

    def _cancel_virtual_midline_picking(self, reset_button=True):
        self._midline_picking = False
        self._midline_edge_picks = []
        self._clear_virtual_midline_temp_highlights()
        if reset_button and hasattr(self, "btn_virtual_midline"):
            self.btn_virtual_midline.blockSignals(True)
            self.btn_virtual_midline.setChecked(False)
            self.btn_virtual_midline.blockSignals(False)
        if hasattr(self, "lbl_status"):
            self._update_status()

    def _cancel_project_edge_face(self, reset_button=True):
        self._projecting = False
        self._project_source_edge = None
        self._clear_project_temp_highlights()
        if reset_button and hasattr(self, "btn_project_edge_face"):
            self.btn_project_edge_face.blockSignals(True)
            self.btn_project_edge_face.setChecked(False)
            self.btn_project_edge_face.blockSignals(False)
        if hasattr(self, "radio_manual") and self.radio_manual.isChecked():
            self._activate_selection_mode(SEL_MODE_EDGE)
        if hasattr(self, "lbl_status"):
            self._update_status()

    def _clear_virtual_midline_temp_highlights(self):
        if not hasattr(self, "viewer"):
            return
        ctx = self.viewer._display.GetContext()
        for ais in getattr(self, "_midline_temp_ais", []):
            try:
                ctx.Remove(ais, False)
            except Exception:
                pass
    ...
    def _clear_project_temp_highlights(self):
        if not hasattr(self, "viewer"):
            return
        ctx = self.viewer._display.GetContext()
        for ais in getattr(self, "_project_temp_ais", []):
            try:
                ctx.Remove(ais, False)
            except Exception:
                pass
    ...
    def _highlight_virtual_midline_reference_edge(self, edge):
        ais = AIS_Shape(edge)
        ais.SetColor(self.COLOR_CYAN)
        ais.SetWidth(6.0)
        ctx = self.viewer._display.GetContext()
        ctx.Display(ais, False)
        self._midline_temp_ais.append(ais)
        self.viewer._display.Repaint()

    def _highlight_project_reference_shape(self, shape):
        ais = AIS_Shape(shape)
        ais.SetColor(self.COLOR_CYAN)
        ais.SetWidth(6.0)
        ctx = self.viewer._display.GetContext()
        ctx.Display(ais, False)
        self._project_temp_ais.append(ais)
        self.viewer._display.Repaint()

    def _pick_edge(self, edge):
        if edge.ShapeType() != TopAbs_EDGE:
            return
        if self._midline_picking:
            self._handle_virtual_midline_edge_pick(edge)
            return
        if self._projecting:
            self._handle_project_source_edge_pick(edge)
            return
    ...
    def _handle_virtual_midline_edge_pick(self, edge):
        length = _edge_length(edge)
        if length < EDGE_MIN_LENGTH:
            QMessageBox.warning(self, "Invalid Edge", "Selected edge is too short for a virtual midline.")
            return
        if self._midline_edge_picks and self._midline_edge_picks[0].IsSame(edge):
            QMessageBox.information(
                self,
                "Same Edge Selected",
                "Virtual Midline needs two different reference edges.\n\n"
                "Please pick the matching edge on the other body."
            )
            self.lbl_status.setText("Virtual Midline:\npick edge 2 of 2")
            self.statusBar().showMessage("Virtual Midline: pick edge 2 of 2")
            return
        self._midline_edge_picks.append(edge)
        self._highlight_virtual_midline_reference_edge(edge)
        pick_count = len(self._midline_edge_picks)
        print(f"[PICK] Virtual midline reference edge {pick_count}: {length:.2f} mm")
        if pick_count < 2:
            self.lbl_status.setText("Virtual Midline:\npick edge 2 of 2")
            self.statusBar().showMessage("Virtual Midline: pick edge 2 of 2")
            return

        edge1, edge2 = self._midline_edge_picks[:2]
        try:
            ok, message = self._validate_virtual_midline_gap(edge1, edge2)
            if not ok:
                QMessageBox.information(self, "Virtual Midline Not Applicable", message)
                return
            wire_info = self._build_virtual_midline(edge1, edge2)
            if wire_info is None:
                raise RuntimeError("Could not build a valid midline wire from the selected edges.")
            self._add_virtual_midline_segment(wire_info, edge1=edge1, edge2=edge2)
        except Exception as exc:
            QMessageBox.warning(self, "Virtual Midline Failed", str(exc))
        finally:
            self._cancel_virtual_midline_picking(reset_button=True)

    def _edge_samples(self, edge, n_samples):
    ...
    def _aligned_edge_samples_for_midline(self, edge1, edge2, n_samples):
        pts1 = self._edge_samples(edge1, n_samples)
        pts2 = self._edge_samples(edge2, n_samples)
        if len(pts1) < 2 or len(pts2) < 2:
            return [], []

        forward_score = _pnt_dist(pts1[0], pts2[0]) + _pnt_dist(pts1[-1], pts2[-1])
        reverse_score = _pnt_dist(pts1[0], pts2[-1]) + _pnt_dist(pts1[-1], pts2[0])
        if reverse_score < forward_score:
            pts2 = list(reversed(pts2))
        return pts1, pts2

    def _validate_virtual_midline_gap(self, edge1, edge2):
        try:
            dist_calc = BRepExtrema_DistShapeShape(edge1, edge2)
            dist_calc.Perform()
            if dist_calc.IsDone():
                min_edge_gap = dist_calc.Value()
                if min_edge_gap <= BRIDGING_MIN_DIST_MM:
                    return (
                        False,
                        "The selected edges are touching or effectively coincident.\n\n"
                        "Virtual Midline is only for real gaps. For touching geometry, use the normal "
                        "auto/shared-edge result or regular Manual edge selection."
                    )
                if min_edge_gap > BRIDGING_MAX_DIST_MM:
                    return (
                        False,
                        f"The selected edges are too far apart for a virtual weld midline "
                        f"(minimum gap {min_edge_gap:.2f} mm)."
                    )
        except Exception as exc:
            print(f"[VIRTUAL] Edge distance precheck failed, falling back to sampled gap check: {exc}")

        pts1, pts2 = self._aligned_edge_samples_for_midline(
            edge1, edge2, VIRTUAL_MIDLINE_SAMPLE_COUNT
        )
        if len(pts1) < 2 or len(pts2) < 2:
            return False, "Selected edges could not be sampled."

        distances = [_pnt_dist(p1, p2) for p1, p2 in zip(pts1, pts2)]
        min_dist = min(distances)
        avg_dist = sum(distances) / len(distances)
        max_dist = max(distances)

        if max_dist <= BRIDGING_MIN_DIST_MM or avg_dist <= BRIDGING_MIN_DIST_MM:
            return (
                False,
                "The selected edges are touching or effectively coincident.\n\n"
                "Virtual Midline is only for real gaps. For touching geometry, use the normal "
        ...
        if min_dist <= POINT_CONTACT_TOL_MM:
            return (
                False,
                "The selected edges touch at one or more sampled points.\n\n"
                "Virtual Midline is only for separated gap edges; use the actual touching edge/segment instead."
            )
        if avg_dist > BRIDGING_MAX_DIST_MM:
            return (
                False,
                f"The selected edges are too far apart for a virtual weld midline "
                f"(average gap {avg_dist:.2f} mm)."
            )
        return True, ""

    def _dedupe_midpoints(self, points, tol=1e-6):
        out = []
        for p in points:
            if not out or _pnt_dist(out[-1], p) > tol:
                out.append(p)
        return out

    def _make_bspline_wire_from_points(self, points):
        arr = TColgp_Array1OfPnt(1, len(points))
        for i, p in enumerate(points, 1):
            arr.SetValue(i, gp_Pnt(p[0], p[1], p[2]))
    ...
    def _make_polyline_wire_from_points(self, points):
        mk = BRepBuilderAPI_MakeWire()
        added = 0
        for i in range(len(points) - 1):
            if _pnt_dist(points[i], points[i + 1]) <= 1e-6:
                continue
            ...
            mk.Add(edge)
            added += 1
        if added == 0 or not mk.IsDone():
            return None
        return mk.Wire()

    def _build_virtual_midline(self, edge1, edge2, n_samples=VIRTUAL_MIDLINE_SAMPLE_COUNT):
        pts1, pts2 = self._aligned_edge_samples_for_midline(edge1, edge2, n_samples)
        if len(pts1) < 2 or len(pts2) < 2:
            return None
        ...
        midpoints = self._dedupe_midpoints(midpoints)
        if len(midpoints) < 2:
            return None

        wire = None
        try:
            wire = self._make_bspline_wire_from_points(midpoints)
        except Exception as exc:
            print(f"[VIRTUAL] BSpline midline failed, falling back to polyline: {exc}")
        if wire is None:
            wire = self._make_polyline_wire_from_points(midpoints)
        if wire is None:
            return None
    ...
    def _find_parent_face_for_edge(self, edge):
    ...
    def _add_virtual_midline_segment(self, wire_info, edge1=None, edge2=None):
        if self.active_weld_id is None:
            self._create_new_weld(name=None, context="virtual_midline")
        active = self._active_weld()
        if active is None:
            return

        face_a = self._find_parent_face_for_edge(edge1) if edge1 is not None else None
        face_b = self._find_parent_face_for_edge(edge2) if edge2 is not None else None
        seg = self._segment_from_wire_info(
            wire_info,
            method_used="Virtual midline",
            face_a=face_a,
            face_b=face_b,
        )
        seg["type"] = "Virtual midline"
        seg["method"] = "Virtual midline"
        seg["virtual"] = True
        seg["reference_faces_found"] = face_a is not None and face_b is not None

        active["context"] = "virtual_midline"
        self._display_segment_for_weld(active, seg)
        self.viewer._display.Repaint()

        self.lbl_path.setText(
            "Virtual midline segment added:\n"
            f"  Length: {seg['length']:.2f} mm\n"
            "  Type: synthetic path for a gap"
        )
    ...
    def _handle_project_source_edge_pick(self, edge):
        length = _edge_length(edge)
        if length < EDGE_MIN_LENGTH:
            QMessageBox.warning(self, "Invalid Edge", "Selected edge is too short to project.")
            return
        self._project_source_edge = edge
        self._highlight_project_reference_shape(edge)
        self._activate_selection_mode(SEL_MODE_FACE)
        self.lbl_status.setText("Project to Face:\nNow pick the target planar face")
        self.statusBar().showMessage("Project to Face: pick the target planar face")
        print(f"[PICK] Projection source edge: {length:.2f} mm")

    def _handle_project_target_face_pick(self, shape):
        if shape.ShapeType() != TopAbs_FACE:
            return
        target_face = shape
        try:
            if self._face_surface_type(target_face) != GeomAbs_Plane:
                QMessageBox.warning(self, "Projection Failed", "Target face must be planar for projection.")
                return
            self._highlight_project_reference_shape(target_face)
            wire_info = self._project_edge_to_planar_face(self._project_source_edge, target_face)
            if wire_info is None:
                raise RuntimeError("Could not build a valid projected wire on the target face.")
            self._add_projected_edge_segment(
                wire_info,
                source_edge=self._project_source_edge,
    ...
    def _project_edge_to_planar_face(self, edge, face, n_samples=VIRTUAL_MIDLINE_SAMPLE_COUNT):
        source_points = self._edge_samples(edge, n_samples)
        if len(source_points) < 2:
            return None

        normal = self._face_normal_at_midpoint(face)
        if normal is None:
            return None
        normal = _vec_normalized(normal)
        if normal is None:
            return None
        ...
        wire = None
        try:
            wire = self._make_bspline_wire_from_points(projected)
        except Exception as exc:
            print(f"[PROJECT] BSpline projection failed, falling back to polyline: {exc}")
        if wire is None:
            wire = self._make_polyline_wire_from_points(projected)
        if wire is None:
            return None
    ...
    def _add_projected_edge_segment(self, wire_info, source_edge=None, target_face=None):
        if self.active_weld_id is None:
            self._create_new_weld(name=None, context="edge_face_projection")
        active = self._active_weld()
        if active is None:
            return

        source_face = self._find_parent_face_for_edge(source_edge) if source_edge is not None else None
        seg = self._segment_from_wire_info(
            wire_info,
            method_used="Edge->Face projection",
            face_a=source_face,
            face_b=target_face,
        )
        seg["type"] = "Projected to face"
        seg["method"] = "Edge->Face projection"
        seg["virtual"] = True
        seg["source_face_found"] = source_face is not None

        active["context"] = "edge_face_projection"
        self._display_segment_for_weld(active, seg)
        self.viewer._display.Repaint()

        self.lbl_path.setText(
            "Projected edge segment added:\n"
            f"  Length: {seg['length']:.2f} mm\n"
            "  Type: synthetic projection onto target face"
        )
    ...
    def _display_segment_for_weld(self, weld, seg):
        ...
        color = self.COLOR_VIRTUAL_MIDLINE if seg.get("virtual", False) else self._segment_color(seg_idx)
        width = 5.0 if seg.get("virtual", False) else 5.0
        ais_list = []
        for w in seg["wires"]:
    ...
    def _method_human_label(self, method):
        ...
        if method == "Virtual midline":
            return "Virtual midline"
        if method == "Edge->Face projection":
            return "Edge to face projection"
    ...
    def _method_context(self, method):
        ...
        if method == "Manual":
            return "manual"
        if method == "Virtual midline":
            return "virtual_midline"
        if method == "Edge->Face projection":
            return "edge_face_projection"
        if method == "Point contact":
    ...
    def _virtual_midline_color_qt(self):
        return QColor(56, 255, 20)
        ...
        for weld in self.welds:
            ...
            for i, seg in enumerate(weld["segments"]):
                if seg.get("virtual", False):
                    color = self.COLOR_VIRTUAL_MIDLINE
                    width = 5.0
                else:
    ...
    def _apply_segment_orientation(self, seg, reversed_order):
        # Invariant: (seg["start_point"] == wire natural start) IFF (reversed_for_continuity == False).
        # When reversed_order=False the caller is saying "keep the segment as it currently is",
        # so we must NOT mutate the flag — start_point/end_point may already have been swapped
        # by a previous auto-order call and the flag is paired with them. The old code reset
        # the flag to False here, breaking the invariant on the 2nd+ auto-order run and
        # producing corner discontinuities (e.g. 150–250 mm gap between L-joint segments).
    ...
    def _rebuild_segment_list(self):
        ...
        for i, seg in enumerate(segments):
            ...
            if seg.get("method") == "Edge->Face projection":
                prefix = "[projected] "
            elif seg.get("virtual", False):
                prefix = "[virtual] "
            else:
                prefix = ""
            text = f"{prefix}Segment {i + 1} - {seg['length']:.1f} mm - {method_label}"
            item = QListWidgetItem(text)
            if seg.get("virtual", False):
                item.setForeground(QBrush(self._virtual_midline_color_qt()))
            else:
                item.setForeground(QBrush(self._segment_color_qt(i)))
            method = seg.get("method", "")
            if method == "Section" or method.startswith("Bridging") or seg.get("virtual", False):
                font = QFont()
                font.setItalic(True)
                font.setBold(True)
                item.setFont(font)
    ...
    def _trajectory_ready_welds(self):
        """Return live OCC geometry in the format expected by trajectory_mapping_1S."""
        ready_welds = []
        skipped = 0
        for weld in self.welds:
            ready = {
        ...
        try:
            ...
            if window is not None:
                try:
                    ...
                    window = None
            if window is None:
                window = trajectory_mapping_1S.launch_with_welds(
                    ready_welds,
                    step_file_path=self.current_step_file,
                    parent=self,
                    step_mm=30.0,
    ...
    def _segments_to_json(self):
        def segment_to_json(i, seg):
            return {
                ...
                "reversed_for_continuity": seg.get("reversed_for_continuity", False),
                "virtual": bool(seg.get("virtual", False)),
            }
    ...
    def _update_status(self):
        ...
        if self.radio_manual.isChecked():
            if self._projecting:
                if self._project_source_edge is None:
                    self.lbl_status.setText("Project to Face:\npick the source edge")
                else:
                    self.lbl_status.setText("Project to Face:\nNow pick the target planar face")
            elif self._midline_picking:
                pick_no = min(len(self._midline_edge_picks) + 1, 2)
                self.lbl_status.setText(f"Virtual Midline:\npick edge {pick_no} of 2")
            else:
