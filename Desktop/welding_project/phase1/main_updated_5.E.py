"""
Robotic Laser Welding — Phase 3   (main_updated_5.E.py)
========================================================
5.B → 5.C eklenenler:
...
BRIDGING_LEN_REL_TOL     = 0.15   # %15 göreceli uzunluk farkı (5.E)
...
# ── Bridging Candidates Dialog (5.E) ────────────────────────────────────────

class BridgingCandidatesDialog(QDialog):
    """
    Clearance fit durumda bulunan bridging adaylarını checkbox'lı listede göster.
    Operatör hangilerini segment olarak eklemek istediğini seçer.
    """

    def __init__(self, candidates, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Clearance Fit — Bridging Candidates")
        ...
        info = QLabel(
            ...
            f"Hangilerini kaynak yolu (segment) olarak eklemek istersiniz?"
        )
        ...
        for i, c in enumerate(candidates):
            text = (
                f"Pair {i + 1} — Body 1: {c['length_a']:.2f} mm  |  "
                f"Body 2: {c['length_b']:.2f} mm  |  "
                f"gap {c['dist']:.2f} mm"
            )
        ...
        warn = QLabel(
            "⚠ Bridging segment'ler doğrulama gerektirir.\n"
            "Otomatik tespit her zaman doğru kaynak yolu olmayabilir."
        )
        warn.setStyleSheet(
...
class MainWindow(QMainWindow):
    """5.E: multi-bridging + checkbox seçim dialog'u."""
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Robotic Laser Welding – Phase 3 (Multi-Bridging)")
    ...
    # ─── ALGO 4: Multi-bridging (5.E) ─────────────────────────────────

    def _find_bridging_candidates(self, face_a, face_b):
        """
        İki face'in tüm edge'lerini tara, uygun edge çiftlerinin LİSTESİNİ
        döndür. Her edge_b en fazla bir edge_a ile eşleşir (matched_b set).

        Eşleşme kriteri:
          • Göreceli uzunluk farkı < BRIDGING_LEN_REL_TOL  (%15)
          • BRIDGING_MIN_DIST_MM ≤ dist ≤ BRIDGING_MAX_DIST_MM
        ...
        for edge_a, length_a in edges_a:
            best_for_a = None  # (b_idx, edge_b, length_b, dist)
            for j, (edge_b, length_b) in enumerate(edges_b):
                if j in matched_b:
                    continue

                # Göreceli uzunluk farkı
                ...
                if dist < BRIDGING_MIN_DIST_MM:
                    continue  # zaten Algo 2'de yakalanır
    ...
    def _apply_bridging_batch(self, candidates, selected_indices):
        """Operatörün checkbox dialog'da seçtiği adayları sırayla
        segment olarak ekle. Her aday için Body A edge'i kullanılır."""
        ...
        for idx in selected_indices:
            ...
            if not wires:
                print(f"[ALGO] Bridging skip pair {idx + 1}: wire build failed")
                continue
            ...
            method_used = f"Bridging (gap {dist:.2f} mm)"
        ...
        # UI'i tek seferde güncelle
        if added > 0:
            self.lbl_path.setText(
                f"⚠ Bridging batch: {added} segment(s) added — verify visually\n\n"
        ...
        self._update_status()
        self._update_status_bar()
        print(f"[ALGO] Bridging batch applied: {added} segment(s) added")
    ...
    # ─── Add Segment (5.E: multi-bridging) ────────────────────────────
    ...
    def add_segment(self):
        ...
        if not edges_info:
            ...
            if gap is not None and gap < CLEARANCE_THRESHOLD_MM:
                ...
                candidates = self._find_bridging_candidates(face_a, face_b)
                print(f"[ALGO] Bridging candidates found: {len(candidates)}")
                for i, c in enumerate(candidates):
                    print(f"  Candidate {i + 1}: len_a={c['length_a']:.2f}, "
                          f"len_b={c['length_b']:.2f}, dist={c['dist']:.2f}")
                ...
                # Adaylar var → checkbox dialog
                dialog = BridgingCandidatesDialog(candidates, parent=self)
                if dialog.exec_() == QDialog.Accepted:
                    selected = dialog.selected_indices()
                    if selected:
                        self._apply_bridging_batch(candidates, selected)
                    return
                else:
                    return
            else:
