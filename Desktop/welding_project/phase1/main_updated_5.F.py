"""
Robotic Laser Welding — Phase 3   (main_updated_5.F.py)
========================================================
5.B → 5.C eklenenler:
...
5.D → 5.E eklenenler:
  • Çoklu bridging: tek edge yerine N adet adayı toplu olarak öner
  • BridgingCandidatesDialog: checkbox'lı seçim arayüzü
  • Length tolerance göreceli (%15) — dairesel clearance fit'lerde de devreye girer
  • Operatör seçtiği adayları tek seferde batch olarak segment listesine ekler

5.E → 5.F eklenenler:
  • Path-based bridging: edge çiftleri yerine "Path A / Path B" konsepti
  • Her path = bir face'in karşı face'e bakan edge'lerinin birleştirilmiş wire'ı
  • Operatör hangi tarafı (Body A ya da Body B) kaynak yapacağını seçer
  • Aynı face çifti ikinci kez seçildiğinde aynı path'ler — bu doğal davranış,
    operatör tarafı değiştirmek istiyorsa farklı face seçer
  • Sabit yüksek-kontrast renk paleti (gradient yerine)
  • Method etiketi: "Bridging-A" veya "Bridging-B" (hangi taraftan)

Algoritma sırası (5.F):
  1) Topological shared-edge (IsSame, TShape pointer)
  2) Geometric coincident-edge (length + endpoint + dist)
  3) Section fallback
  4) Path-based bridging proposal (operatör Path A / Path B seçer)
...
# Clearance fit / bridging
CLEARANCE_THRESHOLD_MM   = 5.0
BRIDGING_MIN_DIST_MM     = 0.05
BRIDGING_MAX_DIST_MM     = 5.0

# Segment color palette (5.F: yüksek kontrast, sabit renkler)
SEGMENT_PALETTE = [
    (0.85, 0.10, 0.10),   # kırmızı
    (0.10, 0.10, 0.10),   # siyah
    (0.10, 0.45, 0.85),   # koyu mavi
    (0.55, 0.20, 0.75),   # mor
    (0.10, 0.55, 0.20),   # koyu yeşil
    (0.85, 0.45, 0.10),   # turuncu
    (0.55, 0.10, 0.45),   # koyu pembe
    (0.20, 0.55, 0.55),   # turkuaz
]
...
# ── Bridging Paths Dialog (5.F) ─────────────────────────────────────────────

class BridgingPathsDialog(QDialog):
    """
    Path-based bridging: iki bağımsız kaynak yolu (Path A / Path B) sun.
    Operatör birini, ikisini veya hiçbirini seçebilir. Default: hiçbiri seçili
    DEĞİL — bilinçli karar zorla.
    """
    ...
    def __init__(self, paths, parent=None):
        ...
        for p in paths:
            ...
            item.setCheckState(Qt.Unchecked)   # 5.F: default unchecked
...
class MainWindow(QMainWindow):
    ...
    # ─── ALGO 4: Path-based bridging (5.F) ────────────────────────────

    def _find_bridging_paths(self, face_a, face_b):
        """
        Clearance fit'te iki bağımsız kaynak path'i çıkar.

        Path A: Face A'nın Face B'ye yakın edge'lerinin birleştirilmiş wire'ı
        Path B: Face B'nin Face A'ya yakın edge'lerinin birleştirilmiş wire'ı

        Returns: list of path dicts (0, 1, veya 2 eleman)
        """
        ...
        for side, face_main, face_other, body_idx in [
            ...
            near_edges = []      # [(edge, length), ...]
            edge_dists = []      # her near edge için mesafe
    ...
    def _apply_bridging_paths(self, paths, selected_indices):
        """
        Operatörün checkbox dialog'da seçtiği path'leri segment olarak ekle.
        Her path = TEK segment (kaç edge'i birleştirmiş olursa olsun).
        """
        ...
        # Face seçimi sıfırla (sadece bir kez)
        if self.face_a is not None:
            ctx.Remove(self.face_a[2], False)
            self.face_a = None
        if self.face_b is not None:
        ...
        sides = [paths[i]['side'] for i in selected_indices]
        print(f"[ALGO] Bridging paths applied: {sides}")
    ...
    # ─── Segment colors (5.F: distinct palette, gradient değil) ───────
    ...
    # ─── Add Segment (5.F: path-based bridging) ───────────────────────
    ...
    def add_segment(self):
        ...
        if not edges_info:
            ...
            if gap is not None and gap < CLEARANCE_THRESHOLD_MM:
                ...
                paths = self._find_bridging_paths(face_a, face_b)
                print(f"[ALGO] Bridging paths found: {len(paths)}")
                for p in paths:
                ...
                dialog = BridgingPathsDialog(paths, parent=self)
                if dialog.exec_() == QDialog.Accepted:
                    selected = dialog.selected_indices()
                    if selected:
                        self._apply_bridging_paths(paths, selected)
                    return
                else:
                    return
            else:
    ...
    # ─── Continuity check (5.F: aynı bridging side connected sayılır) ─

    def _segment_gap(self, seg_a, seg_b):
        method_a = seg_a.get("method", "")
        method_b = seg_b.get("method", "")
        # Aynı side'dan ardışık iki bridging segmenti → connected
